...
public class MembershipDocument : IDocument
{
    public List<User> Model { get; }
    public MembershipDocument(List<User> model)
    {
        Model = model;
    }
    ...
    void ComposeHeader(IContainer container)
    {
        ...
        container.Row(row =>
        {
            ...
            {
                column.Item().Text($"Membership List").Style(titleStyle);
    ...
    void ComposeTable(IContainer container)
    {
        container.Table(table =>
        {
            ...
            {
                table.Cell().Element(CellStyle).Text(item.Id.Substring(0, 8));
                ...
                table.Cell().Element(CellStyle).AlignRight().Text($"{item.PhoneNumber}");
